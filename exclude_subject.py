...
for index in range(first_trial_indx,len(edfdata)):
    
    trl_counter = True # counter to update good trials
    
    if edfdata[index]['events']['Ssac']: # if saccade list not empty
        sacc_dist = [] # to save saccade distances for trial
        for _,sac in enumerate(edfdata[index]['events']['Esac']): # for each saccade in trial
